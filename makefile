...
# C compile
CCFLAGS=OPTIMIZE(10) ENUM(*INT) TERASPACE(*YES) STGMDL(*INHERIT) SYSIFCOPT(*IFSIO) INCDIR($(INCLUDE)) DBGVIEW($(DBGVIEW)) TGTCCSID($(CCSID))
...
%.rpgle:
	-system -q "CRTSRCPF FILE($(BIN_LIB)/QRPGLESRC) RCDLEN(112) CCSID($(CCSID))"
	liblist -a $(LIBLIST);\
...
%.sqlrpgle:
	...
	system -iK "CRTSQLRPGI OBJ($(BIN_LIB)/$(notdir $*)) SRCSTMF('src/$*.sqlrpgle') RPGPPOPT(*LVL2) COMPILEOPT('$(SQLRPGCFLAGS)') DBGVIEW(*NONE) COMMIT(*NONE)" ;\

%.cmd:
	system -q "CHGATR OBJ('cmd/$*.cmd') ATR(*CCSID) VALUE(1252) CCSID($(CCSID))"
	-system -q "CRTSRCPF FILE($(BIN_LIB)/QCMDSRC) RCDLEN(132) CCSID($(CCSID))"
...
%.clle:
	-system -q "CRTSRCPF FILE($(BIN_LIB)/QCLLESRC) RCDLEN(112) CCSID($(CCSID))"
	liblist -a $(LIBLIST);\
...
%.dspf:
	...
	-system -q "CRTSRCPF FILE($(BIN_LIB)/QDDSSRC) RCDLEN(132) CCSID($(CCSID))"
